...
static int get_peak_pos(const kiss_fft_cpx *cout, int nfft, int start_pos, float sample_hz) {
	int pos = 0;
	float maxdata = 0;
	int i;
	for (i = start_pos; i < (10 * nfft / sample_hz); i++) { // Loop till 10 steps "Very fast :)"
	...
	if (pos != 0)
	{	
		//printf("*********** Steps amplitude is: %f ************* \n", (maxdata / (pos * sample_hz / nfft)));
		if ((maxdata / (pos * sample_hz / nfft)) < 40)
		{
			pos = 0;
		}
	}
...
static float get_peak_frequence(const kiss_fft_cpx *cout, int nfft, float start_hz, float sample_hz) {
	...
	return get_peak_pos(cout, nfft, start_pos, sample_hz) * sample_hz / nfft;
}
...
float data_processor_run(data_processor_t dfft) {
    ...
    return get_peak_frequence(dfft->cout, dfft->nfft, 0.0, dfft->sample_hz);
}
