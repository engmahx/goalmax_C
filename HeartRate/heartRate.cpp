...
void heart_get_one()
{
	...
	while(sample_index < max_samples)
	{
		...
		//if ((current_reading > 2)||(current_reading < 0))
		...
		if(sample_index > (adc_window_size + dev_window_size + 1))
		{
			...
			if((integration_window[0] < integration_window[1]) && (integration_window[1] > integration_window[2]))
			{
				...
				else if(beaks_found == 1)
				{
					...
					///* mahx test code */ printf("(sample_index - adc_window_size) is:%d\n",(sample_index - adc_window_size));

					// Calculate the new deviation threshold
					dev_threshold = threshold_buffer / (sample_index - adc_window_size);

					if(dev_threshold < ther_min)
					{
						dev_threshold = ther_min;
					}
					//else
					//{
						// Calculate heart BPM
						///* mahx test code */ printf("(second_beak_time - first_beak_time) is:%lld\n",(second_beak_time - first_beak_time));
						if((second_beak_time - first_beak_time))
...
int heart_init(void)
{
	...
	adc = new ads1015(adc_i2c, 0x48);
			 ...
			 */
			adc->setConfigRegister(0x4283); // 0x42E3

			uint16_t c_reg = adc->getConfigRegister();
			printf("ADC configure register %x\n", c_reg);
			if (c_reg != 0x4283) // (c_reg != 0x4283) && (c_reg != 0x8583)
			{
	...
	// *************** Debug *******************
	#ifdef debug_heart
		myfile = fopen("/home/goalmax/BPM_test_local.csv","w");
	#endif
